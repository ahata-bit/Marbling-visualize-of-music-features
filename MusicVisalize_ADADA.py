...
# メインウィンドウの初期化後にサブウィンドウを開く
root.after(200, open_save_window)
...
"""
        ...
        canvas.create_rectangle(
            ...
            outline="black", width=3, fill=rgb_to_hex(genre_color)
        )
...
print("多数決による予測ジャンル:", genre_list[predicted_label])
print("各区間の予測:", [genre_list[i] for i in results])

# ジャンル名もウィンドウ左上に表示
canvas.create_text(10, 10, text=f"{genre_list[predicted_label]}  BPM:{bpm:.2f}", fill="white", font=("Arial", 20, "bold"), anchor="nw")

root.after(100, initialize_marbling_with_colors)
root.mainloop()
